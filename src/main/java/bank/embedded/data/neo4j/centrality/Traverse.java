...
public class Traverse {
 ...
 public Traverse(GraphDatabaseService graphdb) {
		this.db=graphdb;
	}    
	...
	public Stream<Path> search(Node node,RelationshipType rel,Direction dir,int depth) 
    {
		...
		//重置层次
		info.put("depth1", 0);info.put("depth2", 0);info.put("depth3", 0);info.put("maxlength", 0);
	  		...
	  		public Evaluation evaluate(Path path) {
	  			...
	  			if(relinuse.size()>count) {
	  				...
	  				if(!nodeinuse.containsKey(path.endNode().getId())) { 
	  				   ...
	  				   if(path.length()==1) info.put("depth1", info.get("depth1")+1);
	  				   if(path.length()==2) info.put("depth2", info.get("depth2")+1);
	  				   if(path.length()==3) info.put("depth3", info.get("depth3")+1);
