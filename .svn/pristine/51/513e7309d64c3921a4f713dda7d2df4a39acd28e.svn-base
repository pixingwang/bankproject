...
function showInfo(){
	...
	$("#nodeSize").html((n>0 ? (n+" 客户&nbsp&nbsp&nbsp&nbsp"):"")+(r>0 ? (r+" 关系&nbsp&nbsp&nbsp&nbsp"):"")+(r+n>0 ? ("其中包括："):"")
...
$(document).ready(function(){
	//关系筛选动态显示
	  $("#shaixuanbtn").click(function(){
		  ...
		  var arr = new Array(6);
		  var r =d3.select("g").selectAll(".link").size()-d3.select("g").selectAll(".link.shadow").size();
		  arr[0]=d3.select("g").selectAll(".link.DB").size();
		  arr[1]=d3.select("g").selectAll(".link.JY").size();
		  arr[2]=d3.select("g").selectAll(".link.DK").size();
		  arr[3]=d3.select("g").selectAll(".link.DY").size();
	...
	//节点筛选动态显示
	  $("#nodeshaixuanbtn").click(function(){
		  ...
		  arr[0]=d3.select("g").selectAll(".node.B").size();
		  arr[1]=d3.select("g").selectAll(".node.P").size();
		  arr[2]=d3.select("g").selectAll(".node.C").size();
		  arr[3]=d3.select("g").selectAll(".node.Com").size()+d3.select("g").selectAll(".node.Per").size();
	     ...
	     for(var i=0;i<4;i++){
	    	 if(arr[i]!=0){//去掉多余的分隔符
	    		 ...
	    		 console.log("i="+i+"  sum="+sum+" n="+r);
	    		 if(r==sum && i<3)//如果没有其它关系，去掉多余的分隔符
	    			 items[2*i+1].style.display='none';
	    	 }
	    	 else{
	    	 	items[2*i].style.display='none';
...
/*			console.log("panel="+$("#testPanel").css("height"));
			...
			$("#testPanel").css("height","90%");
		}
	
	});
	$("#closePanel").click(function(){
		$("#nodePanel").animate({left:"-100%"},"slow");
		$("#showPanel").animate({left:"-30%"},"slow");//与显示节点信息面板的宽度(15%)相同 
		$("#nodeInfo").empty();
	});
	
	/*$("#listView").click(function(){
		$("#list").animate({right:"0%",opacity: 'show'},"slow");
	});
...
function showTable(tag){
	   ...
	   if(tag == 1){//交易关系的表格
		   ...
		   console.log("担保环统计");
		   isTrue = true;
		   //var dbdata = {"DbCircleInfo":[{"nodesNum":2,"count":3},{"nodesNum":2,"count":3},{"nodesNum":2,"count":3},{"nodesNum":2,"count":3}]};
		   ...
		   console.log("列表显示");
		   if(/*$("#listTable").hasClass("dataTable")*/$.fn.DataTable.isDataTable( '#listTable' )){
			   console.log("已经初始化了");
			   return;
		   }
		   console.log("开始初始化");
...
function getTotalNumOfDBByCount(n,isInput){
	var dbdata = {"DbCircleInfo":[{"nodesNum":2,"count":2},{"nodesNum":3,"count":3},{"nodesNum":4,"count":4},{"nodesNum":5,"count":5},{"nodesNum":15,"count":100}]};
	var dblen;//担保环长度
	if(isInput){
	...
	dbdata.DbCircleInfo.forEach(function(d){
		if(d.nodesNum == dblen){
			totalPage = Math.ceil((d.count+1)/numPerPage);
		}
	});
}
...
function setSkip(id){
   ...
   switch(id){
	...
	switch(tag){
		...
		case "当前显示：贷款用于银承保证金":searchYDZC(skipValue);break;
		case "当前显示：贷款资金回流至担保人":searchDkhlzdbr(skipValue);break;
		case "当前显示：信贷资金违规进入房地产行业":searchDkzrfdc(skipValue);break;
		case "当前显示：信贷资金流入证券、期货":searchDkzrrzdb(skipValue);break;
		case "当前显示：信贷资金回流至法人代表账户":searchDkhlzfrdb(skipValue);break;
		case "当前显示：向本行员工发放经营性贷款":searchDkffzyg(skipValue);break;
		case "当前显示：贷款以新还旧":searchDkyjhx(skipValue);break;
		case "当前显示：消费贷资金违规进入房地产行业":searchXfdjrfdc(skipValue);break;
...
function model(){
	...
	temp =  "当前显示：";
	var index=$(" option:selected")[0].index;
	switch(index){
		case 1:searchYDZC();totalPage=Math.ceil(totalNum.dkyyycbz/numPerPage);temp = temp+"贷款用于银承保证金";modelfade();break;
		case 2:searchDkhlzdbr();totalPage=Math.ceil(totalNum.dkhlzdbr/numPerPage);temp = temp+"贷款资金回流至担保人";modelfade();break;
		case 3:searchDkzrfdc();totalPage=Math.ceil(totalNum.dkzrfdc/numPerPage);temp = temp+"信贷资金违规进入房地产行业";modelfade();break;
		case 4:searchDkzrrzdb();totalPage=Math.ceil(totalNum.dkzrrzdb/numPerPage);temp = temp+"信贷资金流入证券、期货";modelfade();break;
		case 5:searchDkhlzfrdb();totalPage=Math.ceil(totalNum.dkhlzfrdb/numPerPage);temp = temp+"信贷资金回流至法人代表账户";modelfade();break;
		case 6:searchDkffzyg();totalPage=Math.ceil(totalNum.dkffzyg/numPerPage);temp = temp+"向本行员工发放经营性贷款";modelfade();break;
		case 7:searchDkyjhx();totalPage=Math.ceil(totalNum.dkyjhx/numPerPage);temp = temp+"贷款以新还旧";modelfade();break;
		case 8:searchXfdjrfdc();totalPage=Math.ceil(totalNum.xfdjrfdc/numPerPage);temp= temp+"消费贷资金违规进入房地产行业";modelfade();break;
	}
}
function modelfade(){
		  $('#jychain').fadeOut(0);
...
$(function(){
   ...
   $('#listTable tbody').on('click', 'td .btn', function () {
	   ...
	   if ( row.child.isShown() ) {// This row is already open - close it
        	...
        	/*var closerows = tr.siblings();
        	//var closerows = tr.siblings().find("span").parents("tr");
        	var len = closerows.length;
        	for(var i = 0;i<len;i++){
        		var closerow = table1.row(closerows[i]);
        		if(closerow.child.isShown()) closerow.child.hide();
        	}*/
        	exit();
        	console.log("1");
			searchDBCircleByList(row.data().nodesNum,row.data().skip);
			console.log("4");
